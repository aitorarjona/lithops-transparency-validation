...
def run_po_batch_fiber(iteration, optim_id, batch_size, rs_seed, noise_std):
    ...
    noise_inds = np.asarray([
        noise.sample_index(random_state, len(theta)) for i in range(batch_size)
    ],
                            dtype='int')
...
class ESOptimizer:
    def __init__(self,
        ...
        if is_candidate == False:
            log_fields = [
                ...
                'eval_returns_mean_proposal_from_others_in_{}'.format(
                    optim_id),
            ]
            log_path = log_file + '/' + log_file.split(
                '/')[-1] + '.' + optim_id + '.log'
            self.data_logger = CSVLogger(
                log_path, log_fields + [
                    'time_elapsed_so_far',
                    'iteration',
                ])
        ...
        self.filename_best = log_file + '/' + log_file.split(
            '/')[-1] + '.' + optim_id + '.best.json'
        self.log_data = {}
    ...
    def pick_proposal(self, checkpointing, reset_optimizer):
        ...
        if checkpointing and self.checkpoint_scores > self.proposal:
            ...
            if self.optim_id != self.proposal_source:
                self.set_theta(self.proposal_theta,
    ...
    def save_policy(self, policy_file, reset=False):
        if self.best_score is not None and self.best_theta is not None:
            with open(policy_file, 'wt') as out:
                json.dump([self.best_theta.tolist(), self.best_score],
                          out,
                          sort_keys=True,
                          indent=0,
                          separators=(',', ': '))
    ...
    def update_dicts_after_transfer(self, source_optim_id, source_optim_theta,
                                    stats, keyword):
        eval_key = 'eval_returns_mean_{}_from_others_in_{}'.format(
            keyword,  # noqa
            self.optim_id)
        if eval_key not in self.log_data.keys():
            self.log_data[eval_key] = source_optim_id + '_' + str(
                stats.eval_returns_mean)
        else:
            self.log_data[eval_key] += '_' + source_optim_id + '_' + str(
                stats.eval_returns_mean)
        ...
        if keyword == 'proposal' and stats.eval_returns_mean > self.transfer_target:
            if stats.eval_returns_mean > self.proposal:
                ...
                self.proposal_source = source_optim_id + (
                    '' if keyword == 'theta' else "_proposal")
    ...
    def update_dicts_after_es(self, stats, self_eval_stats):
        ...
        self.log_data.update({
            ...
            'accept_theta_in_{}'.format(self.optim_id):
            'self'
        })

    def broadcast_theta(self, theta):
        '''On all worker, set thetas[this optimizer] to theta'''
        logger.debug('Optimizer {} broadcasting theta...'.format(
            self.optim_id))
    ...
    def add_env(self, env):
        ...
        logger.debug('Optimizer {} add env {}...'.format(
            self.optim_id, env.name))
    ...
    def delete_env(self, env_name):
        ...
        logger.debug('Optimizer {} delete env {}...'.format(
            self.optim_id, env_name))
    ...
    def compute_grads(self, step_results, theta):
        ...
        po_theta_max = theta + noise_sign * self.noise_std * noise.get(
            po_noise_ind_max, len(theta))
    ...
    def start_theta_eval(self, theta):
        ...
        eval_tasks = self.start_chunk_fiber(run_eval_batch_fiber,
    ...
    def get_theta_eval(self, res):
        ...
        logger.debug(
            'get_theta_eval {} finished running {} episodes, {} timesteps'.
            format(self.optim_id, len(eval_returns), eval_lengths.sum()))
    ...
    def start_step(self, theta=None):
        ...
        step_results = self.start_chunk_fiber(run_po_batch_fiber,
    ...
    def get_step(self,
        ...
        if not propose_only:
            ...
            if decay_noise:
                self.noise_std = max(self.noise_std * self.noise_decay,
                                     self.noise_limit)
        ...
        logger.debug('Optimizer {} finished computing gradients'.format(
            self.optim_id))
    ...
    def update_pata_ec(self, archived_optimizers, optimizers, lower_bound,
                       upper_bound):
        ...
        for source_optim in archived_optimizers.values():
            raw_scores.append(
                cap_score(self.evaluate_theta(source_optim.theta)))
